...
### 通过 Codex 插件进行对抗式代码审查

CLAUDE.md 包含 **Code Review** 规则：无论是用户要求还是 skill（如 `code-reviewer`、`simplify`）触发的代码审查，Claude 都会调用 [Codex 插件](https://github.com/openai/codex-plugin-cc) 的 `/codex:adversarial-review` 命令。该插件生成 Codex 审查者，使用不同批判视角，产出跨模型对抗分析和结构化裁决。

需要安装 Codex 插件（默认包含）并设置环境变量 `OPENAI_API_KEY`。
