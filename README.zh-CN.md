...
**Review** 分组可在 [adversarial-review](https://github.com/poteto/noodle)（跨模型，派 Codex 审查者）和 [Codex CLI](https://github.com/openai/codex-plugin-cc)（Codex 插件内置）之间选择，二者**互斥**——选中一个自动取消另一个。
...
| Review (3) | code-review 插件、adversarial-review、Codex CLI | code-review + adversarial-review 开启 |
...
| DeepXiv — Academic Research (3) | deepxiv-cli、deepxiv-trending-digest、deepxiv-baseline-table | 全部关闭 |
...
| **[deepxiv-cli](https://github.com/DeepXiv/deepxiv_sdk)** | arXiv/PMC 论文搜索与阅读 CLI 技能。混合搜索（BM25 + 向量）覆盖 200 万+ 论文，逐章节阅读，内置 AI Agent 分析。安装时从 GitHub 拉取最新版本。 |
| **[deepxiv-trending-digest](https://github.com/DeepXiv/deepxiv_sdk)** | 生成热门学术论文的 Markdown 摘要（近 7 天）。自动化流程：拉取趋势 → 概览每篇 → 排名 → 深读精选 → 结构化报告。 |
| **[deepxiv-baseline-table](https://github.com/DeepXiv/deepxiv_sdk)** | 从论文构建 Baseline 对比表。系统化搜索、筛选、提取基准数据，生成结构化 Markdown 表格。 |
...
- **[Codex CLI](https://github.com/openai/codex-plugin-cc)**：Codex 插件内置审查。需要设置 `OPENAI_API_KEY` 环境变量。不可用时回退到 `code-reviewer` agent。
